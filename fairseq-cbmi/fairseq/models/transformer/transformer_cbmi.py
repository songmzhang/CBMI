...
@register_model("transformer_cbmi")
class TransformerCBMI(TransformerModelBase):
    """
    ...
    def build_model(cls, args, task):
        """Build a new model instance.
           Copy build_model content from TransformerBase and additionally add LM part
        """

        # make sure all arguments are present in older models
        cbmi_architecture(args)
        ...
        # LM part
        if cfg.share_transformer_lm_decoder_embed:
            ...
            nn.init.normal_(
                lm_decoder_softmax_weight.weight, mean=0, std=cfg.decoder.output_dim ** -0.5
            )
        elif cfg.share_lm_decoder_softmax_embed:
        ...
        elif cfg.share_transformer_lm_decoder_softmax_embed:
            lm_decoder_embed_tokens = decoder_embed_tokens
            lm_decoder_softmax_weight = nn.Linear(
                lm_decoder_embed_tokens.weight.shape[1],
                lm_decoder_embed_tokens.weight.shape[0],
        ...
        lm_decoder = TransformerDecoderBase(cfg, 
                                            tgt_dict, 
                                            lm_decoder_embed_tokens, 
                                            no_encoder_attn=True, 
                                            output_projection=lm_decoder_softmax_weight)
        
        return cls(cfg, encoder, decoder, lm_decoder)
    ...
    # Override forward() in TransformerBase and add LM part
    def forward(
        self,
        src_tokens,
        src_lengths,
...
@register_model_architecture("transformer_cbmi", "transformer_cbmi")
def cbmi_architecture(args):
...
@register_model_architecture("transformer_cbmi", "transformer_cbmi_big")
